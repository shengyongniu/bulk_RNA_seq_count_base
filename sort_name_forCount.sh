...
FILE_COUNT=`find /restricted/projectnb/chengcancer/Simon/project/Ho_lab/0raw/SN0126773/*.bam | sed 's/.bam$//'| wc -l`
echo "FILE_COUNT=${FILE_COUNT}"

qsub -P $PROJECT -N $JOBNAME -pe omp $NCORE_REQ -l mem_total=${MEM_LIMIT} -l h_rt=${RT_LIMIT} -t 1-${FILE_COUNT} sort_name_forCount.qsub
